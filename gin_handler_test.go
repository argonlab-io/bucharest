...
func (suite *GinHandlerTestSuite) TestGinHandler() {
	// Arrange
...
func (suite *GinHandlerTestSuite) TestNewGinHandlerFuncWithData() {
	// Arrange
	ctx := NewContextWithOptions(nil)
	suite.Ctx = ctx
	handler := func(ctx HTTPContext) HTTPError {
	...
	// Assert
	suite.assertOkResponse(resp, err)

	bodyBytes, err := io.ReadAll(resp.Body)
	assert.NoError(suite.T(), err)
	assert.Equal(suite.T(), "foobar", string(bodyBytes))

}

func (suite *GinHandlerTestSuite) TestHandlerInfo() {
	// // Arrange
	ctx := NewContextWithOptions(nil)
	suite.Ctx = ctx
	handler := func(ctx HTTPContext) HTTPError {
		assert.Len(suite.T(), ctx.HandlerNames(), 2)
		assert.Equal(suite.T(), ctx.HandlerNames()[0], "UnamedHandler")
		assert.Equal(suite.T(), ctx.HandlerNames()[1], ctx.HandlerName())
		assert.Equal(suite.T(), "MyHandler", ctx.HandlerName())
		ctx.Status(http.StatusNoContent)
		return nil
	}
	...
	// Act
	resp, err := http.Get(suite.Paths[1])

	// Assert
	suite.assertNoContentResponse(resp, err)
}

func (suite *GinHandlerTestSuite) TestFullPath() {
	// Arrange
	ctx := NewContextWithOptions(nil)
	suite.Ctx = ctx
	...
	handler := func(ctx HTTPContext) HTTPError {
		...
		assert.Equal(suite.T(), fmt.Sprintf("/%s", handlerPath), ctx.FullPath())
		ctx.Status(http.StatusNoContent)
		return nil
	}
...
func (suite *GinHandlerTestSuite) TestIp() {
	// Arrange
	ctx := NewContextWithOptions(nil)
	suite.Ctx = ctx

	handler := func(ctx HTTPContext) HTTPError {
		assert.Equal(suite.T(), ctx.Gin().ClientIP(), ctx.ClientIP())
		remoteIp := ctx.RemoteIP()
		expectedRemoteIp := ctx.Gin().RemoteIP()
		assert.Equal(suite.T(), remoteIp, expectedRemoteIp)
		ctx.Status(http.StatusNoContent)
		return nil
	}
...
func (suite *GinHandlerTestSuite) TestCookie() {
	// Arrange
	ctx := NewContextWithOptions(nil)
	suite.Ctx = ctx
	...
	// Act
	client := &http.Client{}
	req, _ := http.NewRequest(http.MethodGet, suite.Paths[0], nil)
...
func (suite *GinHandlerTestSuite) TestContentType() {
	// Arrange
	ctx := NewContextWithOptions(nil)
	suite.Ctx = ctx
	...
	handler := func(ctx HTTPContext) HTTPError {
		...
		assert.Equal(suite.T(), contentType, gin.MIMEJSON)
		ctx.Status(http.StatusNoContent)
		return nil
	}
...
func (suite *GinHandlerTestSuite) TestHeaders() {
	// Arrange
	ctx := NewContextWithOptions(nil)
	suite.Ctx = ctx
	...
	handler := func(ctx HTTPContext) HTTPError {
		...
		assert.Equal(suite.T(), "my-header", myHeader)
		ctx.Status(http.StatusNoContent)
		return nil
	}
...
func (suite *GinHandlerTestSuite) TestRawData() {
	ctx := NewContextWithOptions(nil)
	suite.Ctx = ctx
	...
	handler := func(ctx HTTPContext) HTTPError {
		...
		assert.NoError(suite.T(), err)
		assert.Equal(suite.T(), data["message"], "foobar")
